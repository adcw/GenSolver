...
const GenSelection = ({ genotype, template_id }) => {
  ...
  const getDesc = () => {
    ...
    return <>{
      genotype.map((allelSet, k_aS) => {
        ...
        const txt = (Number(gene.allels[allelSet[0]].prior) === Number(gene.allels[allelSet[1]].prior)
          && allelSet[0] !== allelSet[1]) ?
          `${gene.allels[allelSet[0]].desc}, ${gene.allels[allelSet[1]].desc}`
  ...
  return (
    <>
      ...
      {
        genotype && state.cross_data.count_list &&
        <>
          ...
          <h6 className='mt-2'>Szansa wystąpienia: <span className={`${getStyle(curr_chance.current = chance())}`}>{`${curr_chance.current}%`}</span></h6>
        </>
      }
