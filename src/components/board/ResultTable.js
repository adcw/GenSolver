...
const ResultTable = ({ crossData, setCrossData, crossResult }) => {
  ...
  useEffect(() => {
    ...
    if (el && !el.hasAttribute("style") && state.cross_data.count_list) {
      const gradientMaker = new GradientMaker(Object.keys(state.cross_data.count_list).length);
      setColormap(
        Object.keys(state.cross_data.count_list).map(v => {
          return {
            fen: v,
    ...
    const onPageSwitchToPunnetSquare = EventEmitter.addListener(E.onPageSwitchToPunnetSquare, () => {
      console.log("Page sqitch");
      if (state.cross_data.count_list) {
        console.log(JSON.stringify(state.cross_data.count_list));
      }
    });
    ...
    const onRestoreDefault = EventEmitter.addListener(E.onRestoreDefault, () => {
  ...
  return (
    <div>
      {
        crossResult.current && state.templates.find((t) => t.id === crossData.template_id) ?
                    ...
                    }
                    {
                      v.map((v1, k1) => {
                        ...
                        return <td style={{ backgroundColor: `${color ?? ""}` }}
                          className={`cell pointer slow-trans ${k === selectionPos[0] && k1 === selectionPos[1] ? "glow" : ""} ${secondarySelections.map(sel => JSON.stringify(sel)).includes(JSON.stringify([k, k1])) ? "glow2" : ""}`} key={k1} onClick={() => crossResultClick(v1, [k, k1])}>
